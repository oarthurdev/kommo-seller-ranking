...
async function testLostLeadsDebug() {
    ...
    try {
        ...
        stages?.forEach((stage) => {
            console.log(
                `   - ${stage.stage_name} (ID: ${stage.stage_id}, Pipeline: ${stage.pipeline_id})`,
            );
        });
        ...
        console.log(
            `✅ Configuração de pipelines: ${JSON.stringify(config?.pipeline_id)}`,
        );
        ...
        console.log(
            `✅ Resultado da RPC: ${rpcResult?.length || 0} etapas encontradas`,
        );
        if (rpcResult && rpcResult.length > 0) {
            rpcResult.forEach((result) => {
                console.log(
                    `   - ${result.etapa_anterior}: ${result.total} leads (Valor: ${result.total_value})`,
                );
            });
        } else {
        ...
        console.log(
            `✅ Leads com custom_fields: ${customFieldsLeads?.length || 0}`,
        );

        customFieldsLeads?.forEach((lead) => {
            console.log(
                `   Lead ${lead.id}: etapa="${lead.etapa}", status=${lead.status_id}`,
            );
            try {
                ...
                if (Array.isArray(fields)) {
                    fields.forEach((field) => {
                        if (
                            ...
                            console.log(
                                `     - ${field.field_name}: ${field.values[0].value}`,
                            );
                        }
                    });
                }
                ...
                console.log(
                    `     - Erro ao parsear custom_fields: ${e.message}`,
                );
            }
        });
    } catch (error) {
